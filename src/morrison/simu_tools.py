...
def clean_simu(file, output_file, perfect=True, ngen=3, fid_present=True):
    ...
    inp.seek(home_pos)
    print(("Found "+str(len(list(parents.keys())))+" individuals in "+file))
    print("Cleaning...")
    for l in inp:
        id = l.split()[id_index]
        ...
        if "0" in parents[id]:
            out.write(l)
            continue
        if not (parents[id][0] in parents and parents[id][1] in parents):
            del(parents[id])
            continue
        ...
        if (perfect) and (not par_mrca == ["NA", "NA"]):
            ...
            if par_mrca[0] <= ngen or par_mrca[0] <= ngen:
                ...
                continue
        out.write(l)
    print(("Wrote "+str(len(list(parents.keys())))+" to "+output_file))
    out.close(); inp.close()
